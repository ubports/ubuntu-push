...
func (s *AccSuite) TestPropsHandler(c *C) {
	...
	New(endp, s.log).propsHandler(accountsSoundIface, map[interface{}]interface{}{"x": "y"}, nil)
	...
	New(endp, s.log).propsHandler(accountsSoundIface, map[interface{}]interface{}{}, nil)
	c.Check(s.log.Captured(), Matches, `(?ms).*ERROR PropertiesChanged 3rd param not a list of properties.*`)
	s.log.ResetCapture()

	// bad type for elements of 3rd,
	New(endp, s.log).propsHandler(accountsSoundIface, map[interface{}]interface{}{}, []interface{}{42})
	...
	New(endp, s.log).propsHandler(accountsSoundIface, map[interface{}]interface{}{}, []interface{}{})
	...
	New(endp, s.log).propsHandler(accountsSoundIface, map[interface{}]interface{}{}, []interface{}{"hi", "there"})
	...
	New(endp, s.log).propsHandler(accountsSoundIface, map[interface{}]interface{}{}, []interface{}{"SilentMode"})
	...
	a := New(testibus.NewTestingEndpoint(nil, condition.Work(true), xOuter), s.log)
	called := false
