...
func (ss *postalSuite) TestTakeTheBustFail(c *C) {
	...
	svc := NewPostalService(ss.bus, nEndp, ss.counterBus, ss.log)
...
func (ss *postalSuite) TestStartFailsOnBusDialFailure(c *C) {
	...
	svc := NewPostalService(bus, ss.notifBus, ss.counterBus, ss.log)
...
func (ss *postalSuite) TestInjectBroadcast(c *C) {
	...
	svc := NewPostalService(ss.bus, bus, ss.counterBus, ss.log)
	//svc.msgHandler = nil
