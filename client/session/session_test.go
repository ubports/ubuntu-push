...
func (cs *clientSessionSuite) TestNewSessionPlainWorks(c *C) {
	sess, err := NewSession("foo:443", nil, 0, 0, "", cs.lvls, cs.log)
	c.Check(sess, NotNil)
	c.Check(err, IsNil)
...
func (cs *clientSessionSuite) TestNewSessionHostEndpointWorks(c *C) {
	sess, err := NewSession("http://foo/hosts", pem, 0, 0, "wah", cs.lvls, cs.log)
	c.Assert(err, IsNil)
...
func (cs *clientSessionSuite) TestNewSessionPEMWorks(c *C) {
	sess, err := NewSession("", pem, 0, 0, "wah", cs.lvls, cs.log)
	c.Check(sess, NotNil)
	c.Assert(err, IsNil)
...
func (cs *clientSessionSuite) TestNewSessionBadPEMFileContentFails(c *C) {
	...
	sess, err := NewSession("", badpem, 0, 0, "wah", cs.lvls, cs.log)
	c.Check(sess, IsNil)
	c.Check(err, NotNil)
}
...
func (cs *clientSessionSuite) TestNewSessionBadLevelMapFails(c *C) {
	...
	sess, err := NewSession("", nil, 0, 0, "wah", ferr, cs.log)
	c.Check(sess, IsNil)
	c.Assert(err, NotNil)
}
...
func (cs *clientSessionSuite) TestGetHostsRemoteError(c *C) {
	sess, err := NewSession("", nil, 0, 0, "", cs.lvls, cs.log)
	c.Assert(err, IsNil)
...
func (cs *clientSessionSuite) TestConnectFailsWithNoAddress(c *C) {
	sess, err := NewSession("", nil, 0, 0, "wah", cs.lvls, cs.log)
	c.Assert(err, IsNil)
	err = sess.connect()
...
func (cs *clientSessionSuite) TestConnectConnects(c *C) {
	...
	c.Assert(err, IsNil)
	defer srv.Close()
	sess, err := NewSession(srv.Addr().String(), nil, 0, 0, "wah", cs.lvls, cs.log)
	c.Assert(err, IsNil)
	err = sess.connect()
	c.Check(err, IsNil)
...
func (s *msgSuite) SetUpTest(c *C) {
	var err error
	s.sess, err = NewSession("", nil, time.Millisecond, time.Hour, "wah", levelmap.NewLevelMap, helpers.NewTestLogger(c, "debug"))
	c.Assert(err, IsNil)
...
func (cs *clientSessionSuite) TestDialWorks(c *C) {
	...
	c.Assert(err, IsNil)
	sess, err := NewSession(lst.Addr().String(), nil, timeout, time.Hour, "wah", cs.lvls, cs.log)
	c.Assert(err, IsNil)
